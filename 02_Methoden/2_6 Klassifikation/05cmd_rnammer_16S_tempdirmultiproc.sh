...
for inf in subdivide_*.fasta; do
	echo "rnammer -S bac -m ssu -f 16S_rRNA_${inf} $inf"
	rnammer -S bac -m ssu -f 16S_rRNA_${inf} $inf &
done 
echo "waiting for rnammer to finish..."
wait
echo "finished rnammer"
#mkdir ${HOME}/delme
#echo "cp 16S_rRNA* ${HOME}/delme/."
#cp 16S_rRNA* ${HOME}/delme/.
echo "cat 16S_rRNA_*.fasta > ${workdir}/16S_rRNA_${currdir}.fasta || exit"
cat 16S_rRNA_*.fasta > ${workdir}/16S_rRNA_${currdir}.fasta || exit
echo "cd ${workdir}"
cd ${workdir}
echo "rm -rf ${tempdir}"
rm -rf ${tempdir}
#rm ${HOME}/delme/16S_rRNA*.fasta
